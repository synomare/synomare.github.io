...
function ArticlePills({ articles, onSelect, disabled }) {
  if (!articles?.length) return null;
  return <div className="ops-article-pills">{articles.slice(0, 8).map(article => <button type="button" key={article.slug} disabled={disabled} onClick={() => onSelect(article.slug)}>{article.title || article.slug}</button>)}{articles.length > 8 ? <span>+{articles.length - 8}</span> : null}</div>;
}
...
function TagsTab({ documents, onSelect, onRenameTag, pendingTagChanges, onSaveTagChanges, onDiscardTagChanges, disabled }) {
  ...
  return <section className="ops-section">
    ...
    {pendingTagChanges ? <div className="ops-pending"><span>{pendingTagChanges} ARTICLES CHANGED LOCALLY</span><div><button type="button" disabled={disabled} onClick={onDiscardTagChanges}>DISCARD</button><button type="button" className="primary" disabled={disabled} onClick={onSaveTagChanges}>SAVE TAG CHANGES</button></div></div> : null}
    ...
    <div className="ops-list">{stats.length ? stats.map(tag => <div className="ops-tag-row" key={tag.name}>
      <div className="ops-row-main"><strong>#{tag.name}</strong><small>{tag.count} ARTICLES · {tag.publicCount} PUBLIC · {tag.draftCount} DRAFT</small><ArticlePills articles={tag.slugs.map(slug => { const article = documents.find(document => document.slug === slug); return { slug, title: article?.title || slug }; })} onSelect={onSelect} disabled={disabled}/></div>
      {editing === tag.name ? <div className="ops-inline-edit"><input autoFocus disabled={disabled} value={replacement} onChange={event => setReplacement(event.target.value)} onKeyDown={event => { if (event.nativeEvent.isComposing || event.keyCode === 229) return; if (event.key === 'Enter') applyRename(); if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); setEditing(''); } }}/><button type="button" disabled={disabled} onClick={applyRename}>APPLY</button><button type="button" onClick={() => setEditing('')}>×</button></div> : <button type="button" className="ops-row-action" disabled={disabled} onClick={() => beginRename(tag.name)}>RENAME</button>}
...
function MediaTab({ documents, onSelect, onUseImage, disabled }) {
  const [query, setQuery] = useState('');
...
function LinksTab({ documents, onSelect, disabled }) {
  ...
  return <section className="ops-section">
    ...
    <div className="ops-list">{rows.length ? rows.map((issue, index) => <LinkIssueRow issue={issue} kind={issue.kind} onSelect={onSelect} disabled={disabled} key={`${issue.kind}-${issue.sourceSlug || issue.slug}-${issue.target || index}`}/>) : <p className="ops-empty">保守が必要なリンクはありません。</p>}</div>
    <p className="ops-note">UNRESOLVEDはリンク元を開いて記事名・slug・aliasを修正してください。AMBIGUOUSは同名記事のalias整理が必要です。</p>
  </section>;
}

export default function OperationsPanel({ open, disabled = false, tab, onTabChange, documents, onClose, onSelect, onUseImage, onRenameTag, pendingTagChanges, onSaveTagChanges, onDiscardTagChanges }) {
  const { dialogRef, onDialogKeyDown } = useDialogFocus(open, '[data-dialog-initial-focus]', onClose);
  const changeTabFromKey = (event, index) => {
  ...
  if (!open) return null;
  return <><div className="operations-backdrop" aria-hidden="true" onClick={onClose}/><aside ref={dialogRef} className="operations-panel" role="dialog" aria-modal="true" aria-labelledby="operations-panel-title" aria-describedby="operations-panel-description" tabIndex={-1} onKeyDown={onDialogKeyDown}>
    <header><div><strong id="operations-panel-title">CONTENT TOOLS</strong><span id="operations-panel-description">TAG / IMAGE / LINK MAINTENANCE</span></div><button type="button" aria-label="記事運用ツールを閉じる" data-dialog-initial-focus onClick={onClose}>×</button></header>
    <nav className="operations-tabs" role="tablist" aria-label="記事運用ツールのタブ">{tabs.map(([value, label], index) => <button type="button" id={`operations-tab-${value}`} role="tab" aria-controls="operations-panel-content" key={value} className={tab === value ? 'active' : ''} aria-selected={tab === value} tabIndex={tab === value ? 0 : -1} onKeyDown={event => changeTabFromKey(event, index)} onClick={() => onTabChange(value)}>{label}</button>)}</nav>
    <div className="operations-scroll" id="operations-panel-content" role="tabpanel" aria-labelledby={`operations-tab-${tab}`}>{tab === 'tags' ? <TagsTab documents={documents} onSelect={onSelect} onRenameTag={onRenameTag} pendingTagChanges={pendingTagChanges} onSaveTagChanges={onSaveTagChanges} onDiscardTagChanges={onDiscardTagChanges} disabled={disabled}/> : tab === 'media' ? <MediaTab documents={documents} onSelect={onSelect} onUseImage={onUseImage} disabled={disabled}/> : <LinksTab documents={documents} onSelect={onSelect} disabled={disabled}/>}</div>
  </aside></>;
}
