...
export default function App() {
  ...
  const publish = useCallback(async () => {
    ...
    if (note.postType === 'photo' ? !note.photo : (!note.title.trim() || !note.body.trim())) { setStatus(note.postType === 'photo' ? '写真を1枚選択してください。' : 'タイトルと本文を入力してください。'); return; }
  ...
  return <div className="app-shell"><header className="editor-top"><div className="editor-brand"><a href="/">SYNOMARE</a><span>NOTES</span></div><select aria-label="記事を選ぶ" value={note.existing ? note.slug : '__new__'} onChange={event => selectNote(event.target.value)}><option value="__new__">＋ NEW NOTE</option>{documents.map(doc => <option key={doc.slug} value={doc.slug}>{doc.title || (doc.postType === 'photo' ? `PHOTO / ${doc.date}` : doc.slug)}</option>)}</select><div className="editor-session"><button aria-label="GitHubから記事を再読み込み" onClick={() => refresh(token)} disabled={busy}>RELOAD</button><button aria-label="ログアウト" onClick={() => setToken('')}>LOG OUT</button></div></header><main className={`workspace is-${note.postType}`}><section className="writing"><div className="document-head"><div className="type-switch" aria-label="投稿タイプ"><button type="button" className={note.postType === 'text' ? 'active' : ''} onClick={() => update({ postType: 'text' })}>TEXT</button><button type="button" className={note.postType === 'photo' ? 'active' : ''} onClick={() => update({ postType: 'photo' })}>PHOTO</button></div><input className="title-input" aria-label="タイトル" value={note.title} placeholder={note.postType === 'photo' ? 'Title / optional' : 'Untitled note'} onChange={event => update({ title: event.target.value })}/><div className="document-meta"><span>{note.date}</span><span>{note.slug}</span><span>{note.postType.toUpperCase()} / CARD {note.cardSize === 'auto' ? `AUTO → ${note.postType === 'photo' ? 'L' : 'S / M'}` : note.cardSize.toUpperCase()}</span></div></div>{note.postType === 'photo' ? <PhotoStage note={note} images={images}/> : <><MarkdownEditor value={note.body} onChange={body => update({ body })} notes={relationNotes} onPublish={publish}/><ImageQueue images={images}/></>}{advanced ? <section className="details"><label>SLUG<input value={note.slug} readOnly={note.existing} onChange={event => update({ slug: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })}/></label><label>DATE<input type="date" value={note.date} onChange={event => update({ date: event.target.value })}/></label><label>CARD SIZE<select value={note.cardSize} onChange={event => update({ cardSize: event.target.value })}><option value="auto">AUTO / CONTENT</option><option value="s">S / SMALL</option><option value="m">M / MEDIUM</option><option value="l">L / LARGE</option></select></label><label>VISIBILITY<select value={note.draft ? 'draft' : 'public'} onChange={event => update({ draft: event.target.value === 'draft' })}><option value="public">PUBLIC</option><option value="draft">GITHUB DRAFT</option></select></label><label className="wide">{note.postType === 'photo' ? 'CAPTION / DESCRIPTION' : 'SUMMARY'}<input value={note.summary} placeholder={note.postType === 'photo' ? '未入力でも公開できます' : excerptFromBody(note.body)} onChange={event => update({ summary: event.target.value })}/></label>{note.postType === 'text' ? <label className="wide">CARD EXCERPT<input value={note.cardExcerpt} placeholder="概要を使用" onChange={event => update({ cardExcerpt: event.target.value })}/></label> : null}<label>TAGS<input value={note.tags.join(', ')} placeholder={note.postType === 'photo' ? '写真' : '未分類'} onChange={event => update({ tags: splitList(event.target.value) })}/></label><label>ALIASES<input value={note.aliases.join(', ')} onChange={event => update({ aliases: splitList(event.target.value) })}/></label></section> : null}<div className="editor-actions"><span className={`save-status ${status.startsWith('ERROR') || status.startsWith('CONFLICT') ? 'error' : ''}`} aria-live="polite">{status}</span><label className={`upload ${imageProcessing ? 'is-busy' : ''}`}>＋ {imageProcessing ? 'PROCESSING…' : note.postType === 'photo' && note.photo ? 'REPLACE PHOTO' : 'IMAGE'}<input type="file" accept={IMAGE_ACCEPT} multiple={note.postType === 'text'} disabled={imageProcessing || busy} onChange={imageInput}/></label><button onClick={() => setAdvanced(value => !value)} aria-expanded={advanced}>DETAILS {advanced ? '−' : '+'}</button><button className="primary" onClick={publish} disabled={busy || imageProcessing}>{busy ? 'SAVING…' : imageProcessing ? 'PROCESSING…' : note.draft ? 'SAVE DRAFT' : 'PUBLISH'} <span>⌘↵</span></button></div></section>{note.postType === 'text' ? <Inspector note={note} documents={documents}/> : null}</main></div>;
}
