...
export default function App() {
  ...
  return <div className="app-shell"><header className="editor-top"><div className="editor-brand"><a href="/">SYNOMARE</a><span>NOTES</span></div><select aria-label="記事を選ぶ" value={note.existing ? note.slug : '__new__'} onChange={event => selectNote(event.target.value)}><option value="__new__">＋ NEW NOTE</option>{documents.map(doc => <option key={doc.slug} value={doc.slug}>{doc.title || doc.slug}</option>)}</select><div className="editor-session"><button aria-label="GitHubから記事を再読み込み" onClick={() => refresh(token)} disabled={busy}>RELOAD</button><button aria-label="ログアウト" onClick={() => setToken('')}>LOG OUT</button></div></header><main className="workspace"><section className="writing"><div className="document-head"><input className="title-input" aria-label="タイトル" value={note.title} placeholder="Untitled note" onChange={event => update({ title: event.target.value })}/><div className="document-meta"><span>{note.date}</span><span>{note.slug}</span><span>CARD {note.cardSize.toUpperCase()}</span></div></div><MarkdownEditor value={note.body} onChange={body => update({ body })} notes={relationNotes} onPublish={publish}/><ImageQueue images={images}/>{advanced ? <section className="details"><label>SLUG<input value={note.slug} readOnly={note.existing} onChange={event => update({ slug: event.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '') })}/></label><label>DATE<input type="date" value={note.date} onChange={event => update({ date: event.target.value })}/></label><label>CARD SIZE<select value={note.cardSize} onChange={event => update({ cardSize: event.target.value })}><option value="s">S / SMALL</option><option value="m">M / MEDIUM</option><option value="l">L / LARGE</option></select></label><label>VISIBILITY<select value={note.draft ? 'draft' : 'public'} onChange={event => update({ draft: event.target.value === 'draft' })}><option value="public">PUBLIC</option><option value="draft">GITHUB DRAFT</option></select></label><label className="wide">SUMMARY<input value={note.summary} placeholder={excerptFromBody(note.body)} onChange={event => update({ summary: event.target.value })}/></label><label className="wide">CARD EXCERPT<input value={note.cardExcerpt} placeholder="概要を使用" onChange={event => update({ cardExcerpt: event.target.value })}/></label><label>TAGS<input value={note.tags.join(', ')} placeholder="未分類" onChange={event => update({ tags: splitList(event.target.value) })}/></label><label>ALIASES<input value={note.aliases.join(', ')} onChange={event => update({ aliases: splitList(event.target.value) })}/></label></section> : null}<div className="editor-actions"><span className={`save-status ${status.startsWith('ERROR') || status.startsWith('CONFLICT') ? 'error' : ''}`} aria-live="polite">{status}</span><label className={`upload ${imageProcessing ? 'is-busy' : ''}`}>＋ {imageProcessing ? 'PROCESSING…' : 'IMAGE'}<input type="file" accept={IMAGE_ACCEPT} multiple disabled={imageProcessing || busy} onChange={imageInput}/></label><button onClick={() => setAdvanced(value => !value)} aria-expanded={advanced}>DETAILS {advanced ? '−' : '+'}</button><button className="primary" onClick={publish} disabled={busy || imageProcessing}>{busy ? 'SAVING…' : imageProcessing ? 'PROCESSING…' : note.draft ? 'SAVE DRAFT' : 'PUBLISH'} <span>⌘↵</span></button></div></section><Inspector note={note} documents={documents}/></main></div>;
}
