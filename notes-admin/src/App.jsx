...
export default function App() {
  ...
  const publish = useCallback(async () => {
    if (!note || busy) return; if (QA_PREVIEW) { setStatus('DESIGN PREVIEW — 公開処理は実行しません。'); return; } if (!note.title.trim() || !note.body.trim()) { setStatus('タイトルと本文を入力してください。'); return; }
  ...
  }, [note, busy, token, baseSha, images, refresh]);
  const imageInput = async event => { const files = [...event.target.files]; if (!files.length) return; const added = files.map(file => ({ file, path: `assets/images/notes/${Date.now()}-${file.name.toLowerCase().replace(/[^a-z0-9._-]+/g, '-')}` })); setImages(current => [...current, ...added]); update({ body: `${note.body}${note.body.endsWith('\n') || !note.body ? '' : '\n'}\n${added.map(image => `![${image.file.name}](/${image.path})`).join('\n\n')}\n` }); event.target.value = ''; };
