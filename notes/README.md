...
iPhoneでは上部の記事選択を使って既存記事／**＋ NEW NOTE**を切り替え、画面下の **＋ IMAGE**、**DETAILS**、**PUBLISH** から主要操作を行います。**DETAILS**は本文の上に開くため、閉じるとすぐ執筆へ戻れます。リンク・バックリンク・関連記事は本文下の **LINKS / CONNECTIONS** を開いたときだけ表示されます。下部操作はセーフエリアを避けて固定され、入力内容はPCと同じく自動退避されます。
