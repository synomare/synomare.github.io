...
5. **＋ IMAGE** で画像を選ぶと、形式と破損の有無を確認してから本文へ記法が挿入されます。HEICはJPEG、SVGや大きな静止画はWeb表示向けのWebPへ自動変換され、公開待ち画像のプレビューが表示されます。画像は公開時にMarkdownと同じGitコミットで `assets/images/notes/` へ保存されます。元のファイル名は記事には表示されません。見えるキャプションを付ける場合だけ、Markdownを `![代替テキスト](/画像パス "表示するキャプション")` の形に編集します。
